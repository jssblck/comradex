...
Claude support adds native account pooling to Comradex's existing relay, account management, and menubar.

The router uses quota-aware selection and controlled account rollover. It does not shim the Claude CLI or launch user conversations. Reset warming invokes the official CLI for a bounded background prompt.
...
Validation covers 503 enabled Rust tests and 39 menubar tests, including browser-login status, preferred/preserved routing, management-endpoint cooldowns, concurrent foreground/background token rotation, and durable reset warming. An isolated CLI smoke test covers configuration checks, settings installation, startup, rejection of foreign requests, status, shutdown, and settings restoration. Formatting, Clippy with warnings denied, and diff checks pass.

Live tests with two distinct subscription accounts verify official browser login, new and resumed native conversations, credential substitution, preference changes, and account continuity across a daemon restart. Real OAuth refresh preserves account/device identity and persists the credential; the native warming function completes a Haiku request with the refreshed token. Partial inference quota observations retain the other shared window instead of erasing previously reported weekly usage.

The opt-in `live_managed_claude_refresh_and_warming` test requires `COMRADEX_LIVE_CLAUDE_HOME` pointing at a Comradex-owned Claude account directory. Running it with `cargo test --locked --lib live_managed_claude_refresh_and_warming -- --ignored` rotates that grant and sends one native Haiku prompt; run it only with explicit authorization for that account. Ordinary test runs skip it. Natural reset timing and quota-triggered cross-account rollover have controlled-test coverage but have not been exercised against a real quota rejection.
...
- Each machine obtains independent login grants. Copying a rotating refresh grant between active daemons creates competing refresh owners; filesystem locks coordinate local processes, not separate computers.
