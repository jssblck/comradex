...
Validation on JessBook: `cargo test --locked` passed 493 enabled tests; the synthetic native-capture replay passed separately; `swift test` passed 38 menubar tests. Formatting, Clippy with warnings denied, and diff checks passed. An isolated CLI smoke test covered `check`, Claude settings install, daemon startup, rejection of foreign requests, status, graceful shutdown, and uninstall restoration. The existing service and real credentials were not changed. The separate live-backend test remains ignored.

The implementation uses ordinary native-tls HTTP/1.1 for inference and rustls for OAuth. Header values and application bytes are preserved subject to the stated edits; HTTP framing, header order, and TLS fingerprints are not identical to a direct Claude connection. There is no CPA-style TLS impersonation. Real subscription login/rollover, all helper and IDE surfaces, direct-only native feature checks, and cross-account signed-context portability remain unverified. These limits must remain visible before release; local mock success is not a measured ban-risk result.
...
Normalize quota observations into account-wide and model/mode-specific availability with independent reset times. Start from real response headers. Idle accounts with stale or absent usage are unknown, not zero percent used. If fresh preflight usage is needed for selection, validate a bounded native-compatible usage query separately; do not send inference probes or enable Comradex's weekly-activation request for Claude.
...
The local implementation and synthetic capture do not establish production account safety. Release validation still needs controlled native two-account evidence for the supported portable request shapes, and an inventory of helper/IDE/direct-only feature behavior. Unverified account-owned shapes remain pinned. This work did not access live credentials, send Anthropic inference traffic, or establish a measured ban rate.
