...
`account preserve <name>` saves an account's quota for use outside Comradex, such as ChatGPT web or cloud work. New work uses other available accounts first and falls back to the preserved account only when needed. Existing conversations keep their account. Use `--pool <name>` to choose a pool or `--clear` to stop preserving. Changes persist and apply without restarting the daemon. An account cannot be both preferred and preserved.
