...
impl App {
    pub(super) async fn handle_claude(
        ...
        let owner = if hard {
            binding.as_ref().map(|b| b.account_id.clone()).or_else(|| {
                pool.members
                    .iter()
        ...
        for attempt in 0..pool.members.len() {
            ...
            if response.status().is_success()
                && ((!count && !self.router.bind(identity_key, &account).await)
...
impl Body for ClaudeBody {
    ...
    fn poll_frame(
        ...
        Pin::new(&mut self.inner)
            .poll_frame(cx)
            .map(|frame| frame.map(|frame| frame.map_err(std::io::Error::other)))
    }
